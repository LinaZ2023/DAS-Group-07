...
The boxplots bove show that the difference between the distribution of the variables *budget* and *year* in each goodtype is samll.
...
```{r, eval = TRUE, out.width = '50%', fig.align = "center",warning = FALSE, fig.cap = '\\label{fig:film_data} The correlation coefficient between variables.'}
...
```{r, eval = TRUE, out.width = '60%', fig.align = "center",warning = FALSE, fig.cap = '\\label{fig:film_data} The proportion of different genres under each type of IMBD'}
...
model2 <- glm(goodtype_flag ~ length + budget + genreComedy + genreDocumentary + genreDrama + genreShort, 
...
\begin{align}
\frac{p}{1-p}\ &= exp(\alpha+ \beta_1 \cdot \textrm{Length}+\beta_2 \cdot \textrm{Budget}+\beta_3 \cdot I_{Comedy}+\beta_4 \cdot I_{Documentary} +\nonumber \\
&\beta_5 \cdot I_{Drama}+\beta_6 \cdot I_{Short}\nonumber)
\end{align}
...
Overall, the budget is positively correlated with the probability that the movie rating is greater than 7.
...
The regression model of Comedy, Documentary, Drama, and Short genre shows that.

* Films with genres "Comedy", "Documentary", "Short" always has a high probability of 'rating greater than 7', which means these kind of movies are more likely to rating greater than 7.

* Films with genres "Drama" has low negative relationship with 'rating greater than 7', which means this kind of movies less more likely to rating greater than 7.
